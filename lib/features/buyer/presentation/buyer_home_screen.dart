...
import 'dart:convert';
import 'dart:io';
import 'package:diginiwas/features/buyer/presentation/save_properties_screen.dart';
import 'package:diginiwas/features/buyer/presentation/buyer_profile_screen.dart';
import 'package:diginiwas/features/buyer/presentation/notification_screen.dart';
...
class HomeScreen extends StatelessWidget {
        ...
        children: [
          Row(
            ...
            children: [
              ...
              SizedBox(width: 10.w),
              Obx(() {
                final localPath = controller.userAvatarLocalPath.value;
                final networkUrl = controller.userAvatarNetworkUrl.value.isNotEmpty
                    ? controller.userAvatarNetworkUrl.value
                    : (controller.dashboardHeader.value?.user?.avatar ?? '');
                ...
                return InkWell(
                  ...
                  child: Container(
                    ...
                    child: (!hasLocal && !hasNetwork)
                        ? Text(
                            controller.buyerName.value.isNotEmpty
                                ? controller.buyerName.value.substring(0, 2).toUpperCase()
                                : 'RH',
                            style: GoogleFonts.poppins(
                              color: const Color(0xFF0F2544),
                              fontSize: 14.sp,
          ...
          InkWell(
            onTap: () => controller.fetchUserLocation(),
          ...
          return Expanded(
            child: GestureDetector(
              ...
              child: Container(
                ...
                height: 78.h,
                decoration: BoxDecoration(
                  color: isSelected ? iconColor.withValues(alpha: 0.14) : bgColor,
                  borderRadius: BorderRadius.circular(16.r),
  ...
  Widget _buildVerifiedAgent() {
    ...
    return _VerifiedAgentsList(
      agents: agents,
      cachedImageBuilder: _cachedImage,
      onProfileTap: _showAgentProfileBottomSheet,
    );
  }
...
class HeroBannerCarousel extends StatefulWidget {
  ...
  const HeroBannerCarousel({super.key, required this.imageUrls});
...
class NewListingCard extends StatefulWidget {
  ...
  const NewListingCard({super.key, required this.item, required this.onTap});
                ...
                child: GestureDetector(
                  onTap: () async {
                    try {
                      ...
                      debugPrint('DEBUG_HEART EXCEPTION: $e');
                    }
                  },
                  child: Obx(() {
...
class AutoSlideAgentCard extends StatelessWidget {
  ...
  const AutoSlideAgentCard({super.key, 
    required this.agent,
...
class _VerifiedAgentsList extends StatefulWidget {
  const _VerifiedAgentsList({
  ...
  final void Function(BuildContext, dynamic) onProfileTap;

  @override
  State<_VerifiedAgentsList> createState() => _VerifiedAgentsListState();
}

class _VerifiedAgentsListState extends State<_VerifiedAgentsList> {
    ...
    return SizedBox(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          final initial = (agent.name?.trim().isNotEmpty == true)
              ? agent.name!.trim().substring(0, 1).toUpperCase()
              : 'A';

          return AutoSlideAgentCard(
            agent: agent,
            initial: initial,
            cachedImageBuilder: widget.cachedImageBuilder,
            onProfileTap: (ctx, ag) => widget.onProfileTap(ctx, ag),
          );
        },
      ),
    );
